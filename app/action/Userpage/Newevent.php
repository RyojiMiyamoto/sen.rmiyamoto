...
class Sample_Action_UserpageNewevent extends Sample_ActionClass
{
    ...
    public function prepare()
    {
        ...
        $eventList = $db->GetRow("SELECT event_name FROM eventlist WHERE event_name = ?", array[$newEventName]);
	...
	do{
	    ...
	    $key = $um->makeRandStr(4) . "-" . $um->makeRandStr(4) . "-" . $um->makeRandStr(4);
            ...
            $chkKey = $db->GetRow("SELECT event_key FROM eventlist WHERE event_key = ?", array[$key]);
        ...
        // イベントリストに追加
        $db->Query("INSERT INTO eventlist (event_name, event_key, event_pass) VALUES(?,?,?)",array[$addEventName, $addEventKey, $addEventPassHash]);
        ...
        // イベントIDを取得
        $eventResult = $db->GetRow("SELECT event_id FROM eventlist WHERE event_key = ?", array[$addEventKey]);
